...
from services.scene_runtime import is_running, mark_end, mark_start
...
async def run_crawl_and_ai(
    scene_id: int,
    caller: Annotated[str | None, Query(description="内部调用标记（agent 调度用）")] = None,
    ...
    if is_running(scene_id, "agent") and (caller or "").strip().lower() != "agent":
        return {"code": 409, "status": "error", "msg": "agent 正在运行中，禁止手动执行 crawl/prefilter/submit"}
    if not mark_start(scene_id, "crawl", meta={"caller": (caller or "manual")}):
        return {"code": 409, "status": "error", "msg": "crawl 正在运行中，请稍后"}
    try:
...
async def submit_llm_for_scene(
    ...
    if is_running(scene_id, "agent") and (caller or "").strip().lower() != "agent":
        return {"code": 409, "status": "error", "msg": "agent 正在运行中，禁止手动执行 crawl/prefilter/submit"}
    if not mark_start(scene_id, "submit", meta={"caller": (caller or "manual")}):
        return {"code": 409, "status": "error", "msg": "submit 正在运行中，请稍后"}
    try:
        dynamic_jobconfig.set(scene_manager.get_dynamic_jobconfig(scene_id))
...
async def prefilter_titles_for_scene(
    ...
    if is_running(scene_id, "agent") and (caller or "").strip().lower() != "agent":
        return {"code": 409, "status": "error", "msg": "agent 正在运行中，禁止手动执行 crawl/prefilter/submit"}
    if not mark_start(scene_id, "prefilter", meta={"caller": (caller or "manual")}):
        return {"code": 409, "status": "error", "msg": "prefilter 正在运行中，请稍后"}
    try:
        dynamic_jobconfig.set(scene_manager.get_dynamic_jobconfig(scene_id))
